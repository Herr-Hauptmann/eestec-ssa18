<div class="container-fluid footer">
    <div class="row">

        <div class="socialMedia col-xs-12 col-md-3">

            <div class="footerNaslov">
                <p>Social Media</p>

            </div>

            <div class="socialWrapper">
                <a target="_blank" class="col-xs-3" href="https://www.facebook.com/SoftSkillsAcademySarajevo/">
                    <img alt="Facebook" class="socialImage img-responsive" src="<?php echo e(asset('img/facebookTest.png')); ?>" />
                </a>

                <a target="_blank" class="col-xs-3" href="https://twitter.com/eestecsarajevo">
                    <img alt="Twitter" class="socialImage  img-responsive" src="<?php echo e(asset('img/twitterTest.png')); ?>" />
                </a>

                <a target="_blank" class="col-xs-3" href="https://www.instagram.com/softskillsacademy_sa">
                    <img alt="Instagram" class="socialImage  img-responsive" src="<?php echo e(asset('img/instagramTest.png')); ?>" />
                </a>

                <a target="_blank" class="col-xs-3" href="https://ba.linkedin.com/in/eestec-lc-sarajevo-347390a0">
                    <img alt="Linkedin" class="socialImage  img-responsive" src="<?php echo e(asset('img/linkedinTest.png')); ?>"  />
                </a>

            </div>
        </div>

        <div class="col-xs-12 col-md-3">

            <div class="footerNaslov">
                <p>Kontakt</p>

            </div>

            <div class="kontaktOsobe">
                <p class="kontaktIme">
                    Zlatan Peleksić
                </p>
                <p class="kontaktPozicija prvaKontakt">
                    Koordinator tima za odnose sa javnošću<br>
                     <b>zlatan.peleksic@softskillsacademy.ba</b>
                </p>
                <p class="kontaktIme">
                    Marijana Udovčić
                </p>
                <p class="kontaktPozicija">
                    Glavna organizatorica<br>
                    <b>marijana.udovcic@softskillsacademy.ba</b>
                </p>
            </div>

				


			</div>

		

	</div>

</div>


<div class="container-fluid subFooter">
	<div class="row">
		<div class="subFooter-wrapper col-xs-12">
<p>Copyright (C) EESTEC LC Sarajevo <?php echo e(date('Y')); ?></p>

</div>
</div>
</div>

</body>
</html>
        

